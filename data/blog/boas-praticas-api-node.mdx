...
Em aplicações Node.js geralmente são utilizados dois tipos de estruturação de diretórios: divisão por funcionalidade e divisão por responsabilidade.
...
Entre essas duas abordagens eu prefiro a estruturação por responsabilidade, seguindo minhas origens no mundo Java, onde essa forma de organização é considerada o padrão. No entanto, não acredito que seja a opção correta, fica a critério da equipe de cada projeto discutir e escolher aquela achar mais pertinente.
...
> [https://tools.ietf.org/html/rfc2616#section-9.6](PUT).
